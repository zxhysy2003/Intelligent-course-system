...
本文面向推荐服务开发和后端联调，说明环境配置、接口契约、模型训练与排障。服务概览见 [推荐服务 README](../recommend-service/README.md)，完整环境启动见 [总操作手册](OPERATION_MANUAL.md)。
...
依赖定义在 [`environment.yml`](../recommend-service/environment.yml)，默认环境名为 `lab_autumn`。
...
核心逻辑位于 [`model.py`](../recommend-service/model.py)：
