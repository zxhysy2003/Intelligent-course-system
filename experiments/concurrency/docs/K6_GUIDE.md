...
本文说明 `experiments/concurrency/k6/` 中压测脚本的安装、参数和断言。命令均从仓库根目录执行，先完成 [实验隔离与准备](EXPERIMENT_GUIDE.md)。
...
- [`con-01-concurrent-enrollment.js`](../k6/con-01-concurrent-enrollment.js)
- 详细方案见 [CON-01 并发选课实验](01-concurrent-enrollment.md)。
...
- [`con-02-concurrent-study-progress.js`](../k6/con-02-concurrent-study-progress.js)
- 详细方案见 [CON-02 并发学习进度与首次完课实验](02-concurrent-learning-progress.md)。
...
- [`con-03-study-idempotency.js`](../k6/con-03-study-idempotency.js)
- 详细方案见 [CON-03 STUDY 请求重复消费幂等实验](03-study-event-idempotency.md)。
...
[`con-03-rollback-retry.js`](../k6/con-03-rollback-retry.js) 将故障实验拆成两个独立阶段：
...
该实验固定使用 `stu_newbie` 的课程 14，与主实验课程 10、11、12 隔离。完整启动、故障开关、k6 和 SQL 命令见 [CON-03 事务回滚后重试](03-study-event-idempotency.md#151-事务回滚后重试)。
...
- [`con-04-single-key-cache.js`](../k6/con-04-single-key-cache.js)：同一用户热缓存、快速冷构建、慢冷构建和双实例共享锁。
- [`con-04-cache-avalanche.js`](../k6/con-04-cache-avalanche.js)：多用户同时冷缓存，以及带抖动的逻辑过期刷新。
...
完整操作和结果记录见 [CON-04 重构后验收](04-recommend-cache-breakdown.md)。
