...
脚本按课程、VU 和迭代号生成不同的 `eventId`，明确表示 100 个请求是 100 个独立事件。
...
由于 CON-02 把每个请求定义为独立事件，脚本还要求每个响应都是 `data.replayed=false`。如果误用上一轮的 `eventId`，即使业务码仍为 `200`，本轮阈值也会失败，避免把幂等回放误判为新的原子累加。
...
CON-02 中的 100 个请求被刻意定义为 100 个不同的学习事件，并使用不同 `eventId`，因此每个请求都应该参与累计并写入行为日志。

如果这 100 个请求是同一个逻辑事件的网络重试，就必须复用同一个 `eventId`。原子更新只负责保证增量不丢失；事件唯一键负责保证同一事件不重复生效。
...
重复消费去重已在 CON-03 中通过 `learning_behavior.event_id` 和 `(user_id, event_id)` 唯一索引实现，并将由单独的 SAME/UNIQUE 负载验证。
