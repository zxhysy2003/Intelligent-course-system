...
本实验的 100 个请求被定义为 100 个不同事件，因此它只验证原子累加，不验证同一事件的重复投递。后续 CON-03 已为 STUDY 增加 `eventId` 和数据库唯一键，下一步使用独立的 SAME/UNIQUE 负载验证幂等性。

当前后续工作是完成 CON-03 并发实测；同时可将 695 秒完课前边界进一步加入需要真实 MySQL 的长期自动化集成测试。
